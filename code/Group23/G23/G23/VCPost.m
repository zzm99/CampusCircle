...
- (void)fabu {
    ...
    [HIService addContentWithTitle:t Detail:c Tags:self.tagarray Images:self.imagearray Callback:^(NSString * state) {
        if([state compare:@"success"] == NSOrderedSame) {
            ...
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"发布失败" message:@"" preferredStyle:UIAlertControllerStyleAlert];
...
- (UIButton*)fabuButton {
    if(_fabuButton == nil) {
        ...
        [_fabuButton setTitle:@"发布" forState:UIControlStateNormal];
